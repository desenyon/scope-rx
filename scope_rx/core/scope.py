...
class ScopeRX:
    ...
    def explain(
        ...
        **kwargs: Any
        ...
        # Create explainer
        explainer: BaseExplainer = explainer_class(self.model, **explainer_kwargs)

        try:
            # Generate explanation
            result = explainer.explain(input_tensor, target_class=target_class, **kwargs)
        finally:
            # Clean up hooks to prevent memory leaks and performance degradation
            # caused by accumulating hooks on the model
            if hasattr(explainer, '_remove_hooks'):
                explainer._remove_hooks()
