...
class AttentionRollout(BaseExplainer):
    ...
    def _compute_rollout(self) -> np.ndarray:
        ...
        for attention in self._attention_maps:
            # Handle dimensions
            # attention shape: (batch, heads, seq, seq) or (batch, seq, seq)

            # Fuse heads if needed
            if attention.ndim == 4:
            ...
            # Remove batch dim (assume batch size 1)
            attention_fused = attention_fused[0]

            # Discard low attention values
            if self.discard_ratio > 0:
                flat = attention_fused.reshape(-1)
            ...
            # Re-normalize
            attention_fused = attention_fused / (attention_fused.sum(dim=-1, keepdim=True) + 1e-8)

            curr_np = attention_fused.cpu().numpy()

            # Accumulate rollout
            if result is None:
                result = curr_np
            else:
                # Matrix multiplication: current layer * previous layers accumulation
                result = np.matmul(curr_np, result)

        if result is None:
             raise RuntimeError("No attention matrices found")
