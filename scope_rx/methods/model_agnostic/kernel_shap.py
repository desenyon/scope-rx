...
class KernelSHAP(BaseExplainer):
    ...
    def _shap_kernel(self, M: int, z: np.ndarray) -> float:
    ...
    def _sample_coalitions(
        ...
        # empty
        coalitions.append(np.zeros(num_segments, dtype=np.float32))
        weights.append(1e6)

        # full
        coalitions.append(np.ones(num_segments, dtype=np.float32))
        weights.append(1e6)

        # Sample random coalitions
        # Note: We need to iterate over the number of samples we want to generate
        # The original code iterated over self.num_samples - 2 but didn't actually generate anything inside the loop properly
        # Fixed logic to generate random coalitions
        ...
        for _ in iterator:
            # Sample coalition
            # Use 0.5 probability for now, but KernelSHAP ideally samples from specific distribution
            z = np.random.binomial(1, 0.5, num_segments).astype(np.float32)
            coalitions.append(z)
            weights.append(self._shap_kernel(num_segments, z))

        coalitions_np = np.array(coalitions)
        weights_np = np.array(weights)

        # Compute predictions for all coalitions
        # We need to compute predictions for the *generated* coalitions
        # The original code iterated over coalitions but was trying to use 'z' which wasn't defined in the loop scope correctly if not careful
        # But here 'coalitions' is a list of arrays, so iterating over it is fine.
        ...
        with torch.no_grad():
            for z in coalitions:
                # Create masked input
                # This is inefficient (looping over pixels), but functional for now.
                # Optimization would involve batching.

                # Vectorized mask creation
                mask = np.isin(segments, np.where(z == 1)[0]).astype(np.float32)

                mask_tensor = torch.from_numpy(mask).unsqueeze(0).unsqueeze(0).to(self.device)

                # Expand mask to match input channels
                mask_tensor = mask_tensor.expand_as(input_tensor)
        ...
        return coalitions_np, weights_np, np.array(predictions_list)
